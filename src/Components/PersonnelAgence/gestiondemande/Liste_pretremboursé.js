...
const Liste_pretremboursé = () => {
    ...
    const renderedArray = data.filter((item) => {
        if (item.statuP == "Rembourser" && item.client.agc.id == localStorage.getItem('id_agence')) {
    ...
    const decaisserFunction = (itemId, clientId, montant_p, pretId) => {
        ...
        axios.post(`http://localhost:8080/transfert/Pret/montant/${clientId}/${pretId}?montant=${montant_p}`,
            {
                "montant": montant_p
            },
    ...
    const renderTableRows = () => {
        ...
        return newArray.slice(startIndex, endIndex).map(row => (
            ...
            <tr key={row.id}>
                ...
                <td><label class="badge badge-success">{row.statuP}</label> </td>
...
export default Liste_pretremboursé;