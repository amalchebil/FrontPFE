...
export default function ListeProjetsRemboursées() {
    const logout = () => {
    ...
    const viewuserFunction = (id) => {
        navigate("/viewprojet/" + id, { state: { id: id } })
    ...
    const renderTableRows = () => {
        ...
        return newArray.slice(startIndex, endIndex).map(row => (
            <tr key={row.id}>
                <td>{row.id}</td>
                <td>{row.activite}</td>
                <td>{row.client.montant}</td>

                <td>{row.gouvernorat}</td>
                <td>{row.secteur}</td>
                <td>  <button type="button" class="btn btn-inverse-info btn-fw" style={{ fontSize: 10 }}> {row.status}</button></td>
                    ...
                    <div className="col-lg-12 grid-margin stretch-card">
                        <div className="card">
                            <div className="card-body">
                                <h4 className="card-title">les projets en jour</h4>
